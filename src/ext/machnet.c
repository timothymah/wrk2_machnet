...
void machnet_detach(const MachnetChannelCtx_t *ctx) {
    ...
    // Release shared memory or perform other cleanup actions if necessary
    munmap(ctx, sizeof(MachnetChannelCtx_t)); // Example for unmapping shared memory
    // Optionally close any associated sockets if opened during the attach process.
}
